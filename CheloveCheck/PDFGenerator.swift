...
final class PDFGenerator {
    static func generatePDF(from receipt: Receipt) -> Data {
        ...
        // Позиции товаров
        checkForNewPage(&currentY, pageHeight: pageHeight, margin: margin)
        ...
        guard let pdfDocument = PDFDocument(data: generatedPDF) else {
            return generatedPDF
        }
        
        let totalPages = pdfDocument.pageCount
        let qrPageIndex = totalPages - 1
    
        guard let qrPage = pdfDocument.page(at: qrPageIndex) else {
            return generatedPDF
        }
        
        let pageBounds = qrPage.bounds(for: .mediaBox)
        ...
        qrPage.addAnnotation(linkAnnotation)
    ...
    // MARK: Считаем длину каждого элементавв
    private static func checkForNewPage(_ currentY: inout CGFloat, pageHeight: CGFloat, margin: CGFloat) {
        if currentY + 50 >= pageHeight {  // 50 - примерное значение запаса перед новым блоком
            UIGraphicsBeginPDFPage()
            currentY = margin
        }
    }
